...
export default function IntervenePage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-bold">Intervention Steps</h1>
      <p className="text-[var(--color-text-muted)]">
        Zero-typing support — tap the button and follow the steps.
      </p>
      ...
      >
        {loading ? "Generating..." : steps.length > 0 ? "Regenerate" : "Get Steps Now"}
      </button>
      ...
      {steps.length > 0 && (
        <ol className="flex flex-col gap-4" aria-label="Intervention steps">
          {steps.map((step, i) => (
